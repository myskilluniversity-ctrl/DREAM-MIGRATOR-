...
export default function AdminPanel({ initialTab = 'leads' }: AdminPanelProps) {
  ...
  const handleExportCSV = () => {
    ...
    const guideLeads = leads.filter(l => l.source === 'download').length;
    const simulationLeads = leads.filter(l => l.source === 'simulation_confirm').length;
    const assessmentLeads = leads.filter(l => l.source === 'assessment').length;
    const contactLeads = leads.filter(l => l.source === 'contact').length;

    const newStatusCount = leads.filter(l => l.status === 'new').length;
    const contactedStatusCount = leads.filter(l => l.status === 'contacted').length;
    const qualifiedStatusCount = leads.filter(l => l.status === 'qualified').length;
    const disqualifiedStatusCount = leads.filter(l => l.status === 'disqualified').length;
    const enrolledStatusCount = leads.filter(l => l.status === 'enrolled').length;

    const totalBudget = leads.reduce((sum, l) => sum + (l.budget || 0), 0);
    const totalPaid = leads.reduce((sum, l) => sum + (l.amountPaid || 0), 0);

    let csvContent = "";
    
    // Section 1: Lead Statistics Header & Metrics (Summary)
    csvContent += "=== SYSTEM & LEAD STATISTICS SUMMARY ===\n";
    csvContent += "Metric,Value\n";
    csvContent += `Total Leads,${totalLeads}\n`;
    csvContent += "Conversion Rate,18.4%\n";
    csvContent += `Guide Downloads,${guideLeads}\n`;
    csvContent += `Simulations,${simulationLeads}\n`;
    csvContent += `Eligibility Assessments,${assessmentLeads}\n`;
    csvContent += `Contact Inquiries,${contactLeads}\n`;
    csvContent += `Status: New,${newStatusCount}\n`;
    csvContent += `Status: Contacted,${contactedStatusCount}\n`;
    csvContent += `Status: Qualified,${qualifiedStatusCount}\n`;
    csvContent += `Status: Disqualified,${disqualifiedStatusCount}\n`;
    csvContent += `Status: Enrolled,${enrolledStatusCount}\n`;
    csvContent += `Financials: Total Budget (INR),${totalBudget}\n`;
    csvContent += `Financials: Total Paid (INR),${totalPaid}\n`;
    csvContent += "\n\n";

    // Section 2: Detailed Users / Leads data
    csvContent += "=== DETAILED USER & LEAD RECORDS ===\n";
    csvContent += "ID,Name,Email,Phone,Source,Status,Budget (INR),Amount Paid (INR),Payment Status,Created At,Updated At,Details\n";
    ...
    leads.forEach(l => {
      ...
      const idVal = escapeCSV(l.id);
      const nameVal = escapeCSV(l.name);
      const emailVal = escapeCSV(l.email);
      const phoneVal = escapeCSV(l.phone);
      const sourceVal = escapeCSV(l.source);
      const statusVal = escapeCSV(l.status);
      const budgetVal = escapeCSV(l.budget ?? 0);
      const amountPaidVal = escapeCSV(l.amountPaid ?? 0);
      const paymentStatusVal = escapeCSV(l.paymentStatus ?? "pending");
      const createdAtVal = escapeCSV(l.createdAt);
      const updatedAtVal = escapeCSV(l.updatedAt);
      
      let detailsString = "";
      if (l.details) {
        try {
          detailsString = JSON.stringify(l.details);
        } catch (e) {
          detailsString = String(l.details);
        }
      }
      const detailsVal = escapeCSV(detailsString);

      csvContent += `${idVal},${nameVal},${emailVal},${phoneVal},${sourceVal},${statusVal},${budgetVal},${amountPaidVal},${paymentStatusVal},${createdAtVal},${updatedAtVal},${detailsVal}\n`;
    });
    ...
    link.href = url;
    link.setAttribute("download", `dream_migrator_analytics_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'leads' && (
        ...
        {activeTab === 'analytics' && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
              <div>
                <h2 className="text-xl font-display font-black text-slate-900 uppercase tracking-tight">System & Lead Analytics</h2>
                <p className="text-slate-500 text-xs font-medium">Export lead statistics, CRM user datasets, and acquisition tracking.</p>
              </div>
              <button 
                onClick={handleExportCSV}
                id="export-csv-button"
                className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 border border-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-slate-900 hover:border-emerald-500 transition-all shadow-md active:scale-95 duration-200"
              >
